...
# =====================================================
# This maps every JSON key the AI extracts to your Standard Row Schema
FIELD_METADATA = {
    # --- ENVIRONMENT ---
    ...
    "revenue": {"pillar": "E", "category": "Energy", "label": "Energy Intensity (Revenue)"}, # Nested under energy_intensity
    "physical": {"pillar": "E", "category": "Energy", "label": "Energy Intensity (Physical)"}, # Nested under energy_intensity
    ...
    "permanent_employees": {"pillar": "S", "category": "Retention", "label": "Attrition (Employees)"}, # Nested under attrition
    "permanent_workers": {"pillar": "S", "category": "Retention", "label": "Attrition (Workers)"}, # Nested under attrition
    "training_hours_per_employee": {"pillar": "S", "category": "Training", "label": "Training Hours"},
    "employees": {"pillar": "S", "category": "Safety", "label": "LTIFR (Employees)"}, # Nested under safety
    "workers": {"pillar": "S", "category": "Safety", "label": "LTIFR (Workers)"}, # Nested under safety
...
def extract_nested_data(pdf_path):
    ...
    # 4. THE FULL RESPONSE SCHEMA
    # This matches the structure we want the AI to fill
...
def parse_value_unit(raw_string):
    ...
    # Regex to find number (handling commas/decimals) at start
...
def transform_to_standard_schema(nested_data):
    ...
    def process_item(key, value, category_override=None):
        if key == "company_metadata": return

        # Recursion for nested dicts (like scope_3_breakdown)
        if isinstance(value, dict):
            for sub_key, sub_val in value.items():
                process_item(sub_key, sub_val, category_override)
            return

        # Lookup Metadata
        mapping = FIELD_METADATA.get(key)
        
        # If key is missing in metadata, default to Generic (so we don't lose data)
        if not mapping:
            mapping = {
                "pillar": "General", 
                "category": "Unmapped", 
...
if __name__ == "__main__":
    ...
    print(f"\n✅ Converted {len(final_data)} data points. Check esg_standardized_output.json")